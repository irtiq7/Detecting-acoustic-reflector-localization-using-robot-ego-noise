function [gainEst,delayEst,response,costFunctionDelay2,estimates]=relaxalgoAfterBF(xOut, setup,signals)
% output -> [gainEst,delayEst,response,costFunctionDelay2, estimates]
signals.signalObservFft = fft(xOut', setup.signal.nfft);
...
for gg = 1:n
    ...
    convergence_array = zeros(length(signals.signalObservFft(:,gg)),1);
    ...
    %     while 1
        ...
        [gainEst(rr,gg),delayEst(rr,gg),response(:,rr,gg),costFunctionDelay2]=...
            delayEstimationFIR_filter(signals.signalObservFft(:,gg), signals.signalCleanFft,response_sim,setup.nlsEst.candidateDelays);
    ...
    %   RELAX algorithm ----> nothing relaxed about it
        if rr>1
            while 1
                ...
                convergence_array(i) = norm(signals.signalObservFft(:,gg) - response_sim.*signals.signalCleanFft);
                i=i+1;
                if i>2
