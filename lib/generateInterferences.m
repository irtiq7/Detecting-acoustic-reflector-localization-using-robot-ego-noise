function signals=generateInterferences(rirs,signals,setup, noiseSel,SNR)
%     rng(0);
    interference= audioread(setup.signal.diffNoiseStr);
    interval=1:setup.signal.lengthSignal*setup.array.micNumber;
    randStartNdx=randi(setup.signal.lengthSignal*setup.array.micNumber,1);
    signals.interference1 = interference(randStartNdx:randStartNdx+length(interval)-1);
    randStartNdx=randi(setup.signal.lengthSignal*setup.array.micNumber,1);
    signals.interference2 = interference(randStartNdx:randStartNdx+length(interval)-1);
    randStartNdx=randi(setup.signal.lengthSignal*setup.array.micNumber,1);
    signals.interference3 = interference(randStartNdx:randStartNdx+length(interval)-1);
    randStartNdx=randi(setup.signal.lengthSignal*setup.array.micNumber,1);
    signals.interference4 = interference(randStartNdx:randStartNdx+length(interval)-1);
%     rng('shuffle');

    signals.directInterference=fftfilt(rirs.direct',signals.clean);
