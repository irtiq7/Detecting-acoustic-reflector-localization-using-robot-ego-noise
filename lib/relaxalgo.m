function [gainEst,delayEst,response,costFunctionDelay2, estimates]=relaxalgo(setup,signals)

[l,n] = size(signals.signalObservFft); %direct path component
...
for gg = 1:n
    ...
    %     while 1
        ...
        [gainEst(rr,gg),delayEst(rr,gg),response(:,rr,gg),costFunctionDelay2]=...
            delayEstimationFIR_filter(signals.signalObservReflFft(:,gg), signals.signalCleanFft,response_sim,setup.nlsEst.candidateDelays);
    ...
    estimates.rNlsEstBeforeBF.toa = delayEst;
    estimates.rNlsEstBeforeBF.gain = gainEst;
end